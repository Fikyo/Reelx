...
from reelx.processors.frame_processor import FrameProcessor
from reelx.processors.input_processor import InputProcessor
from reelx.processors.ffmpeg_processor import FFMPEGProcessor
...
class VideoProcessor:
    def __init__(self, source_path, output_path, mode="vertical"):
        ...
        # Set video mode in configuration
        if self.mode == "square":
        ...
        self.input_processor = InputProcessor(self.source_path)
    ...
    def process_video(self):
        ...
        self.visualizer.set_debug_props("Source WxH", 
            f"{int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))}")    
        
        target_height, target_width = FrameProcessor.get_target_dimensions(
        ...
        try:
            ...
            while cap.isOpened():
                ...
                if Config.OUTPUT_VIDEO_PARAMS["CLASS_ID"] is None:
                    debug_frame, processed_frame = self.frame_processor.process_frame(
                        frame, self.mode, frame_time, frame_count)
                else:
                    print("Processing for the class id : ", Config.OUTPUT_VIDEO_PARAMS["CLASS_ID"])
                    debug_frame, processed_frame = self.frame_processor.process_frame(
                        frame, self.mode, frame_time, frame_count, Config.OUTPUT_VIDEO_PARAMS["CLASS_ID"])
                
                paused, frame_count = self.visualizer.show_frame(
                    debug_frame, processed_frame, fps, total_frames, frame_count, paused)
            ...
            if self.output_path is not None:
                if self.is_live_output:
                    ...
                    self._close_writer()
                    ffmpeg_processor = FFMPEGProcessor(self.source_path, self.output_path)
                    ffmpeg_processor.generate_final_vertical_video()
    ...
    def _setup_ffmpeg_live_stream(self, fps, width, height):
        ...
        try:
            command = [
                ...
                '-f', 'mpegts' if self.output_path.lower().startswith(('udp:', 'rtp:')) else 'mpegts',
                self.output_path
            ]
    ...
    def _setup_writer(self, fps, target_height, target_width):
        """Set up video writer for output file"""
        out = cv2.VideoWriter(
            self.output_path, 
